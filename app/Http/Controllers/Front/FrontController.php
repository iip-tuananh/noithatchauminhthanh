...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $data['categoriesProduct'] = Category::query()->with(['image'])->orderBy('sort_order')->get();
        ...
        $data['projects'] = Project::query()->with(['image'])
            ->where('status', 1)
            ->latest()->get();
        $data['reasons'] = Achievement::query()->with(['image'])->orderBy('created_at', 'asc')->get();
        $data['partners'] = Partner::query()->with(['image'])->orderBy('created_at', 'asc')->get();
    ...
    public function getProductDetail(Request $request, $slug = null) {
        $product = Product::query()->with(['image', 'galleries.image'])
            ->where(['status' => 1, 'slug' => $slug])->first();
        $category = Category::query()->where('id', $product->cate_id)->first();
        $products = Product::query()->with(['image'])
            ->where(['status' => 1, 'cate_id' => $category->id])->whereNotIn('id', [$product->id])->latest()->get();

        return view('site.product_detail', compact('product', 'category', 'products'));
    }
